...
Page({
  ...
  async onRegister() {
    ...
    if (!regPassword || regPassword.length < 8) return wx.showToast({ title: '密码至少8位', icon: 'none' });
    if (!/[a-zA-Z]/.test(regPassword) || !/[0-9]/.test(regPassword)) return wx.showToast({ title: '密码需包含字母和数字', icon: 'none' });
  ...
  async onResetPassword() {
    ...
    if (!resetPassword || resetPassword.length < 8) return wx.showToast({ title: '密码至少8位', icon: 'none' });
    if (!/[a-zA-Z]/.test(resetPassword) || !/[0-9]/.test(resetPassword)) return wx.showToast({ title: '密码需包含字母和数字', icon: 'none' });
